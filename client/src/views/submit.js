...
// This is for BeautifulDnD Component to create a draggable animation
...
class Submit extends Component {
  ...
  renderReviewButton() {
    if (this.state.tagsValid === true && this.state.postTitle !== '' && this.state.postDescription !== '' && this.state.objectives.length > 0) {
      return (
        <Grid item >
          <Button style={{ border: this.props.theme[0].PrimaryBorder, background: this.props.theme[0].PrimaryLinear, textTransform: 'none', color: 'white', float: 'right', height: 35, letterSpacing: '-0.5px', fontSize: '14px', fontWeight: 350, fontFamily: "-apple-system,BlinkMacSystemFont,\"Segoe UI\",Helvetica,Arial,sans-serif,\"Apple Color Emoji\",\"Segoe UI Emoji\",\"Segoe UI Symbol\"" }} onClick={() =>
            this.submitPost(
              ...
              {this.state.objectives.map((obj, index) => (
                <Draggable key={obj.title} draggableId={obj.title + 1} index={index}>
                  {(provided, snapshot) => (
                    ...
                    >
                      <Accordion multiple={true} style={{ maxHeight: '100%' }}>
                        <AccordionTab key={obj.title} header={obj.title} style={{ maxHeight: '100%' }}>
                      ...
                      <Grid container alignItems="center" direction="row" justify="space-between" style={{ marginTop: 8 }}>
                        <Grid item >
                          <Button style={{ background: 'transparent', border: '1px solid #3d63ff', height: 36 }} onClick={() => this.removeObjective(index)}>
                            <div style={{ color: "#3d63ff", textTransform: 'none', letterSpacing: '-0.5px', fontSize: '12px', fontWeight: 350, fontFamily: "-apple-system,BlinkMacSystemFont,\"Segoe UI\",Helvetica,Arial,sans-serif,\"Apple Color Emoji\",\"Segoe UI Emoji\",\"Segoe UI Symbol\"" }}   >
                              <b>Remove</b>
                            </div>
                          </Button>
                        </Grid>
                        <Grid item >
                          <Button style={{ background: this.props.theme[0].PrimaryLinear, border: this.props.theme[0].PrimaryBorder, height: 36 }} onClick={() => this.editObjective(index)}>
                            <div style={{ color: 'white', textTransform: 'none', letterSpacing: '-0.5px', fontSize: '12px', fontWeight: 350, fontFamily: "-apple-system,BlinkMacSystemFont,\"Segoe UI\",Helvetica,Arial,sans-serif,\"Apple Color Emoji\",\"Segoe UI Emoji\",\"Segoe UI Symbol\"" }}>
                              <b>Edit</b>
                            </div>
                          </Button>
                        </Grid>
  ...
  renderPrompt(){
    if(this.state.postPublished === false){
      return(
        <Prompt
          key='block-nav-postPublished'
          when={this.state.postPublished === false}
          message='You have unsaved changes, are you sure you want to leave?'
        />
      )
    } else if (this.state.objectivesEmpty === false){
      return(
        <Prompt
          key='block-nav-objectivesEmpty'
          when={this.state.objectivesEmpty === false}
          message="There's something still in Objectives textbox. Are you sure you want to publish this post?"
        />
      )
    } else if (this.state.confirmPublish === false){
      return(
        <Prompt
          key='block-nav-confirmPublish'
          when={this.state.confirmPublish === false}
